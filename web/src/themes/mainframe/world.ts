...
export function createWorld(mount: HTMLElement, antialias: boolean, powerPref: WebGLPowerPreference | undefined, ratio: number, msaa: boolean): World {
  ...
  scene.fog = new FogExp2(0x04141a, 0.0008);
  ...
  // a near plane of 1, not 0.5: from up high the depth buffer needs the precision for parts that sit close together
  const camera = new PerspectiveCamera(58, window.innerWidth / window.innerHeight, 1, 1500);
